    ...
    fetch(categoryApiUrl)
        ...
        .then(data => {
                        ...
                        .then(getBasin => {
                            ...
                            if (getBasin['assigned-locations']) {
                                getBasin['assigned-locations'].forEach(loc => {
                                    ...
                                    (() => {
                                        let ratingIdApiUrl = setBaseUrl + `ratings/metadata?office=${office}&page-size=200`;
                                        if (ratingIdApiUrl) {
                                            ratingIdPromises.push(
                                                fetch(ratingIdApiUrl)
                                                    .then(response => {
                                                        if (response.status === 404) {
                                                            console.warn(`Datman TSID data not found for location: ${loc['location-id']}`);
                                                            return null;
                                                    ...
                                                    .then(ratingIdData => {
                                                        // console.log("ratingIdData", ratingIdData);

                                                        if (ratingIdData) {
                                                            ratingIdMap.set(loc['location-id'], ratingIdData);

                                                            const filteredRatingIdProductionData = ratingIdData["rating-metadata"].filter(metadata =>
                                                                metadata["rating-spec"].version === "Production" &&
                                                                metadata["rating-spec"]["template-id"].startsWith("Stage;Flow")
                                                            );
                                                            // console.log(filteredRatingIdProductionData);
                                                            ratingIdProductionMap.set(loc['location-id'], filteredRatingIdProductionData);
                                                        }
                                                    })
                                                    .catch(error => {
                                                        console.error(`Problem with the fetch operation for stage TSID data at ${ratingIdApiUrl}:`, error);
                                                    })
                                            );
                                        }
                                    })();
            ...
            Promise.all(apiPromises)
                ...
                .then(() => Promise.all(ratingIdPromises))
                .then(() => {
                    combinedData.forEach(basinData => {
                        if (basinData['assigned-locations']) {
                            basinData['assigned-locations'].forEach(loc => {
                                ...
                                // Add rating id to json
                                const ratingIdMapData = ratingIdMap.get(loc['location-id']);
                                if (ratingIdMapData) {
                                    loc['rating-id'] = ratingIdMapData;
                                }

                                // Add rating id to json
                                const ratingIdProductionMapData = ratingIdProductionMap.get(loc['location-id']);
                                if (ratingIdProductionMapData) {
                                    loc['rating-id-production'] = ratingIdProductionMapData;
                                }
